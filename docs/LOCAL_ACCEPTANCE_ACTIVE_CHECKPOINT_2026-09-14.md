...
- 已按最新10路标准将已验收变更提交为07d073f791f0d5d490ff849c06532ebf83763eaa，在独立干净main副本aural-publish-20260915快进，并推送origin/main成功。原主发布工作区的未提交文档未触碰。
- deploy/release.ps1 -Apply原流程已通过仓库pre-deploy门禁、507项自动回归、23项串行完整页面回归（可选并发项默认0而跳过，10路结果独立留档）、类型/lint基线和正式构建。没有20路运行。
- 制品613008907字节，SHA256前缀85bf0960cbe9e0ce，正在经跳板上传；本段不代表已切换线上。日志output/deploy-20260915-07d073f.log。
- HR按生产6ac9c22独立只读诊断；现有quality-release主分支801f7f2的三个未发布提交保留不变。姓名五种合成格式错误已复现；线上设置页及后端单模型策略拒绝切换也已确认。诊断文档D:\GGGG\kiro\oprun-hr-identity-review-20260915\docs\HR_IDENTITY_MODEL_DIAGNOSIS_2026-09-15.md。没有写生产候选人资料或更改模型配置。

### 本轮发布已完成（2026-09-15 15:02 后复核）

- 发布脚本退出0。线上版本07d073f791f0d5d490ff849c06532ebf83763eaa；不可变制品SHA256=85bf0960cbe9e0ce86e98a7076fbb8836950d87f4bd300863a7f8a6e5a6ac17e。远程main、制品及公网版本一致。
- 版本切换后真实GLM readiness调用成功（23.478秒），服务active，health_ready、公网页面、版本标记、主语音WebSocket握手均PASS。启动过程曾短暂连接失败/503，最终就绪后通过，没有放宽门禁。备用语音未配置，不冒称已通过备用链路。
- 独立复核公网/api/version为目标SHA；真实浏览器/login显示完整邮箱/密码/登录/注册链接。原生产版本d176cdb7fba414f4d7ba9a17d55389cc9545618b及部署快照保留供回滚。
- 10路已通过证据沿用；本轮不运行20路。此前本地真实八题报告及HR四维评分验收沿用，没有重跑或冒称本次生产完整面试验收。
- 最终收据：output/deployment-final-verification-20260915-07d073f.json；完整发布日志：output/deploy-20260915-07d073f.log。
- HR姓名与模型问题已完成同线上版本诊断和合成复现，尚未修改/部署HR，也没有改动生产候选人信息。详细缺陷与建议修复范围见独立HR诊断文档。
...
本轮纠正脚本为可配置非负整数，不启动20路测试。原产品未增加10路限流；此前错误限制在验收工具层。

### 2026-09-16 模型策略本地修复结果

用户澄清：10路为本机验收规模，非容量/脚本上限；已撤销0或10白名单，未运行20路。HR手动点击模型保存、生产失败转备用、隔离测试GLM锁已实现；HR后端64项、缓存5项、UI2项、类型/正式构建均通过，Aural路由34项及类型基线通过。详见 D:\GGGG\kiro\oprun-hr-identity-review-20260915\docs\MODEL_SWITCH_ACCEPTANCE_2026-09-16.md。故障注入为本地替身，不冒称真实备用厂商或线上端到端验收。本轮没有推送或部署，也未混入既有姓名、语音、报告WIP。
