...
直连唯一生产环境。它会写入一条获授权测试申请，因此默认拒绝运行，不能作为本地发布门。

运行前必须在当前任务中获得生产 E2E、所用简历材料和指定岗位授权，并显式提供：
依据 2026-09-06 单项批准，原件和已有脱敏副本均可，不强制脱敏；批准变量与指纹用于
限定样本，不用于要求脱敏。不得向简历本人发出测试通知，模拟结果不代表其真实表现。
...
RESUME_INDEX=7 \
PRODUCTION_RESUME_TEXT_SHA256=<获批简历文本的64位SHA-256> \
...
- 走生产真实数据：每次全量跑会新增测试候选人（不要求简历必须脱敏）；
